...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([dashboardData, historyData]) => {
        if (dashboardData.success) setStats(dashboardData);
  ...
  const trendData = [
    ...
    { month: "Mar", score: 58 },
    { month: "Apr", score: 66 },
    { month: "May", score: stats.average_score || 74 },
    { month: "Jun", score: stats.highest_score || 82 },
  ];

  const pieData = [
    { name: "Ready", value: stats.highest_score || 65 },
    { name: "Improve", value: Math.max(15, 100 - (stats.highest_score || 65)) },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="absolute right-10 top-10 hidden rounded-full border px-4 py-2 text-sm font-extrabold text-[color:var(--accent-green)] md:block"
        ...
        <p className="mt-6 max-w-3xl text-lg leading-8 text-secondary">
          Monitor startup validation performance, track recent ideas, inspect system health,
          and get AI-powered business recommendations from one beautiful workspace.
        </p>

        <div className="mt-8 flex flex-wrap gap-3">
          <Pill icon={Rocket} label="Launch-ready insights" />
          <Pill icon={Brain} label="AI scoring engine" />
          <Pill icon={Database} label="MongoDB live records" />
        </div>
      </motion.section>

      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard i={1} icon={Lightbulb} label="Total Ideas" value={stats.total_ideas} sub="Ideas analyzed" />
        <MetricCard i={2} icon={TrendingUp} label="Average Score" value={stats.average_score} sub="Validation average" />
        <MetricCard i={3} icon={Flame} label="Best Score" value={stats.highest_score} sub="Top idea performance" />
